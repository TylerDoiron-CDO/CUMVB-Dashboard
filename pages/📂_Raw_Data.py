...
if athlete_df.empty:
    ...
    st.markdown("### 🔍 Athlete Record Summary")
    st.write("Total records loaded:", athlete_df.shape[0])
    st.write("Historical records:", (athlete_df["source_file"] == "historical data").sum())
    st.write("Recent records:", athlete_df[athlete_df["source_file"] != "historical data"].shape[0])
    st.write("Unique seasons:", sorted(athlete_df["Season"].dropna().unique()))
